...
'        Call Set_Revise
        ...
        Me.stTime.Enabled = True
        Me.steadyShow.BackColor = &HC0C0C0
        stdAverage = 0
        recflag = True
        If (zeroFlag) Then Call function_Click(0)
        Me.rdTime.Enabled = True
...
Private Sub isSteady(value As Double)
    If (value < 9.9E+35) Then
        ...
        down = stdAverage - rangeN * dataDim.aSteady(VIO, (rangeNum Mod (10 ^ (2 * VIO + 3))) / 10 ^ (2 * VIO + 1), 0)
        up = stdAverage + rangeN * dataDim.aSteady(VIO, (rangeNum Mod (10 ^ (2 * VIO + 3))) / 10 ^ (2 * VIO + 1), 1)
